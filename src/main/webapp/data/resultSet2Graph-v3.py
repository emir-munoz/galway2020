...
for word in spam:
    print "Removing word from hash_nodes=", word
    #print [d for d in hash_nodes if d["name"] == word], vocabulary.index(word)
    hash_nodes[:] = [d for d in hash_nodes if d["name"] != word]
    #hash_nodes[:] = [{"name": "g2020", "group": 4, "radius": "10", "id": "10000"} if d["name"] == word else d for d in hash_nodes]
    vocabulary.remove(word)
    # don't remove word replace it for another word
    #vocabulary = ["#g2020" if x == word else x for x in vocabulary]
...
with open('hash_tags_d3/HashtagLinks.json') as json_data_links:
	...
	if label1 in vocabulary:
	    #print label1,[d for d in hash_nodes if d.get("name") == label1][0]["id"]
	    aLink["source"] = vocabulary.index(label1)
	    #aLink["source"] = [d for d in hash_nodes if d.get("name") == label1][0]["id"]
	else:
	    continue
	...
	if label2 in vocabulary:
	    aLink["target"] = vocabulary.index(label2)
	    #aLink["target"] = [d for d in hash_nodes if d.get("name") == label2][0]["id"]
	else:
	    continue
...
# remove spam
#for word in spam:
    #print "Removing word from hash_links=", word
    #idWord = [d for d in hash_nodes if d.get("name") == word][0]["id"]
    ## remove links where spam word is the source
    #hash_links[:] = [d for d in hash_links if d["source"] != idWord]
...
print "Number of nodes=",len(hash_nodes)
print "Number of links=",len(hash_links)
